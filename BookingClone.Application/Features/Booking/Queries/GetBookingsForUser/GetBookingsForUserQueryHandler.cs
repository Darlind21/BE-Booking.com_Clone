...
namespace BookingClone.Application.Features.Booking.Queries.GetBookingsForUser
{
    public class GetBookingsForUserQueryHandler 
        ...
        : IRequestHandler<GetBookingsForUserQuery, Result<List<BookingResponseDTO>>>
    {
        public async Task<Result<List<BookingResponseDTO>>> Handle(GetBookingsForUserQuery request, CancellationToken cancellationToken)
        {
            var bookings = await bookingRepository.GetBookingsByUserId(request.UserId);
